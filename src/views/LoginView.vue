    ...
    <main class="login-main">
      <section class="intro-panel">
        ...
        <div class="intro-overlay">
          ...
          <p class="intro-foot">严谨 · 高效 · 可追溯</p>
        </div>
      </section>
...
const termsSections = [
  {
    title: '一、服务说明',
    body: '本系统为本地桌面应用，面向科研实验提供数据导入、路径计算与地图可视化等功能。系统功能随版本迭代可能调整，具体以实际界面为准。'
  },
  {
    title: '二、账号使用',
    body: '本系统账号由管理员统一分配与管理，仅限本人使用，严禁转借、共享或泄露给任何第三方。'
  },
  {
    title: '三、账号安全',
  ...
  {
    title: '四、使用规范',
    body: '您在使用本系统过程中需遵守法律法规及公序良俗，不得利用本系统从事违法或损害他人权益的行为，不得恶意干扰系统运行或破坏数据。'
  },
  {
    title: '五、数据责任',
    body: '您应对导入本系统的数据（含坐标、路网、地图配置等）的合法性与准确性负责，确保其来源合法、用途正当；因数据问题产生的后果由您自行承担。'
  },
  {
    title: '六、第三方服务',
    body: '本系统部分功能（如行政区域边界、在线地图底图、地图 API 距离计算）依赖第三方在线服务，使用相关功能时需同时遵守对应服务商的条款与政策。'
  },
  {
    title: '七、行为监督',
  ...
  {
    title: '九、条款更新',
    body: '本条款可根据系统功能与实际情况更新，更新后继续使用本系统即视为接受新条款。'
  }
]
...
const privacySections = [
  {
    title: '一、我们收集的信息',
    body: '本系统收集您在使用过程中主动提供的信息，包括：账号信息（用户名/昵称）与登录密码，以及您在系统内导入或录入的数据（项目信息、起终点坐标、路网文件、地图配置、计算结果等）。我们不会收集与系统运行无关的个人敏感信息。'
  },
  {
    title: '二、密码与凭证安全',
    body: '您的密码经 bcrypt 算法加盐哈希后存储，系统中任何位置均不保存明文密码。您配置的地图 API Key 仅保存在本地数据库中，仅在您发起计算时用于向对应地图服务商鉴权，不用于其他用途。'
  },
  {
    title: '三、数据存储位置',
    body: '您的项目、坐标、计算结果等业务数据保存在您本机（或您指定的）MySQL 数据库中，由您自行管理。'
  },
  {
    title: '四、联网与第三方服务',
    body: '本系统为本地应用，但在使用以下功能时需要联网：行政区域边界（向阿里云 DataV 请求行政区划，仅发送城市编码）；在线地图底图（加载高德地图瓦片，按当前地图视野请求）；地图距离计算（按您填写的平台与 Key，将起终点坐标发送至高德/百度/腾讯/天地图等对应服务商用于计算）。以上通信仅传输功能所必需的少量数据。'
  },
  {
    title: '五、登录会话',
  ...
  {
    title: '六、信息共享',
    body: '除上述功能所必需、向对应地图服务商传输必要数据外，我们不会将您的任何个人信息或业务数据出售、出租或共享给任何第三方。'
  },
  {
    title: '七、您的权利',
    body: '您有权查看与修改本人资料，可删除自己录入的数据。如对个人信息处理有疑问，可联系系统管理员，1509054114@qq.com。'
  }
]
